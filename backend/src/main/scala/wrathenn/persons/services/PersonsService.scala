...
import wrathenn.persons.model.{ApiException, Person, PersonCreate}

trait PersonsService[F[_] : Async] {
  ...
  def editUser(id: Long, person: PersonCreate): F[Either[ApiException, Person]]
...
class PersonsServiceImpl[F[_] : Async](
  ...
  override def editUser(id: Long, person: PersonCreate): F[Either[ApiException, Person]] = {
